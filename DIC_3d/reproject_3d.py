...
for idx in range(1, num_pairs):
    ...
    if filtered:
        ...
        plt.hist(mags, bins=50, color='royalblue', alpha=0.8)
        plt.xlabel('Displacement Magnitude (mm)')
        plt.ylabel('Count')
        plt.title(f'Displacement Histogram {idx}')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'displacement_hist_{idx}.png'))
        plt.close()
    else:
        with open(os.path.join(output_dir, f'displacement_stats_{idx}.txt'), 'w') as f:
            f.write('无有效位移点\n')
    # 新增：绘制未过滤前的位移模长分布直方图
    if displacements:
        mags_raw = np.array([mag for (_, _, _, mag) in displacements])
        import matplotlib.pyplot as plt
        plt.figure(figsize=(6,4))
        plt.hist(mags_raw, bins=50, color='orange', alpha=0.7)
        plt.xlabel('Displacement Magnitude (mm)')
        plt.ylabel('Count')
        plt.title(f'Raw Displacement Histogram {idx}')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'displacement_hist_raw_{idx}.png'))
        plt.close()
    # 新增：绘制未过滤前的位移模长分布直方图（0.5~1.5区间）
    if displacements:
        mags_raw = np.array([mag for (_, _, _, mag) in displacements])
        import matplotlib.pyplot as plt
        plt.figure(figsize=(6,4))
        plt.hist(mags_raw, bins=50, color='green', alpha=0.7, range=(0, 20))
        plt.xlabel('Displacement Magnitude (mm)')
        plt.ylabel('Count')
        plt.title(f'Raw Displacement Histogram (0.5~1.5) {idx}')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'displacement_hist_raw_zoom_{idx}.png'))
        plt.close() 